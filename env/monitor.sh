...
#  - Foreground activity + UDP + CPU in-game detection
#  - Fixed CPU subshell bug
#  - Cached account detection
#  - Automatic rejoin when leaving the game
#  - Detailed in-game detection debugging
# ============================================================
...
RECOVERY_WAIT=10
REJOIN_COOLDOWN=30
OUT_OF_GAME_GRACE=15
UI_CHECK_INTERVAL=3
ACCOUNT_CACHE_TTL=10

# Detection thresholds
UDP_CHECK_ENABLED=1
CPU_INGAME_THRESHOLD=25
...
get_foreground_activity() {
    ...
    CURRENT=$(
        ...
        grep -m 1 -E 'mResumedActivity|mCurrentFocus'
    )
...
is_roblox_foreground() {
    ...
    case "$FOREGROUND" in
        "$TARGET_PACKAGE/$TARGET_RUNNING_ACTIVITY")
...
get_foreground_debug() {
    FOREGROUND=$(get_foreground_activity)

    if [ "$FOREGROUND" = "N/A" ] || [ -z "$FOREGROUND" ]; then
        echo "foreground=N/A roblox_foreground=0"
        return
    fi

    if is_roblox_foreground; then
        echo "foreground=$FOREGROUND roblox_foreground=1"
    else
        echo "foreground=$FOREGROUND roblox_foreground=0"
    fi
}
...
get_udp_debug() {
    ...
    UDP4=$(count_conns_for_uid /proc/net/udp "$UID")
    UDP6=$(count_conns_for_uid /proc/net/udp6 "$UID")

    if [ "$UDP4" -gt 0 ] || [ "$UDP6" -gt 0 ]; then
        echo "udp=1 udp4=$UDP4 udp6=$UDP6 uid=$UID"
    else
        echo "udp=0 udp4=$UDP4 udp6=$UDP6 uid=$UID"
    fi
}
...
print_detection_debug() {
    ...
    echo "[detect] foreground=${LAST_FOREGROUND_ACTIVITY} roblox_foreground=${LAST_FOREGROUND_IS_ROBLOX} cpu=${ROBLOX_CPU_PCT}% udp=${LAST_UDP} state=${GAME_STATE} reason=${LAST_DETECTION_REASON}"
}
...
if not accounts:
    ...
    for m in obj_re.finditer(normalized):
        ...
        if uid_m and name_m:
            picker_m = re.search(
                r'"showInAccountPicker"\s*:\s*(true|false)',
                blob
            )

            signout_m = re.search(
                r'"signOutTimestamp"\s*:\s*(\d+)',
                blob
            )

            signin_m = re.search(
                r'"signInTimestamp"\s*:\s*(\d+)',
                blob
            )
            ...
            push(
                ...
                uid_m.group(1),
                show_picker=(
                    picker_m and picker_m.group(1) == 'true'
                ),
                sign_in=(
                    int(signin_m.group(1))
                    if signin_m else 0
                ),
                sign_out=(
                    int(signout_m.group(1))
                    if signout_m else None
                ),
            )

accounts = [
    a for a in accounts
    if a["sign_out"] is None
]
...
for node in tree.iter('node'):
    ...
    if login is None and 'EditText' not in cls:
        if (
            re.search(r'\blog ?in\b', combo)
            or 'sign in' in combo
            or 'signin' in combo
            or 'btn_login' in combo
            or 'button_login' in combo
        ):
            c = center(bnd)
...
if not username and not password and len(edit_fields) >= 2:
    c0 = center(edit_fields[0][1])
    c1 = center(edit_fields[1][1])
...
if username:
    print(f"USERNAME {username[0]} {username[1]}")

if password:
    print(f"PASSWORD {password[0]} {password[1]}")

if login:
    print(f"LOGIN {login[0]} {login[1]}")
PY
}
...
auto_login() {
    ...
    [ -z "$LINE" ] && {
        echo "Auto-login: accounts file is empty."
        return 1
    }
    ...
    ui_dump || {
        echo "Auto-login: uiautomator dump failed."
        return 1
    }
...
send_join_intent() {
    echo "[join] Sending Roblox game intent for game $ROBLOX_GAME_ID..."
    ...
    if [ "$RESULT" -eq 0 ]; then
        echo "[join] Game join intent sent successfully."
    else
        echo "[join] Game join intent failed with exit code $RESULT."
    fi
...
wait_for_game_start() {
    ...
    while [ "$COUNT" -lt "$JOIN_TIMEOUT" ]; do
        ...
        if is_in_game "$CPU_NOW" "$UDP_NOW"; then
            ...
            echo "Detection reason: ${LAST_DETECTION_REASON}"

            return 0
        fi

        if [ "$DETECTION_DEBUG" -eq 1 ]; then
            echo "[join-detect] foreground=${LAST_FOREGROUND_ACTIVITY} roblox_foreground=${LAST_FOREGROUND_IS_ROBLOX} cpu=${CPU_NOW}% udp=${UDP_NOW} reason=${LAST_DETECTION_REASON}"
        fi
...
force_rejoin() {
    ...
    while [ "$COUNT" -lt "$JOIN_TIMEOUT" ]; do
        if ! is_process_alive; then
            echo "Roblox died during recovery."
        ...
        if [ "$DETECTION_DEBUG" -eq 1 ]; then
            echo "[recovery-detect] foreground=${LAST_FOREGROUND_ACTIVITY} roblox_foreground=${LAST_FOREGROUND_IS_ROBLOX} cpu=${CPU_NOW}% udp=${UDP_NOW} reason=${LAST_DETECTION_REASON}"
        fi
    ...
    if ! is_process_alive; then
        ...
        echo "Recovery join timed out."
        echo "Current foreground: ${LAST_FOREGROUND_ACTIVITY}"
        echo "Current CPU: ${ROBLOX_CPU_PCT}%"
        echo "Current UDP: ${LAST_UDP}"
        echo "Detection reason: ${LAST_DETECTION_REASON}"
    fi
    ...
    "embeds": [
        {
            ...
            "fields": [
                ...
                {"name": "Foreground","value": "${LAST_FOREGROUND_ACTIVITY}","inline": false}
            ],
...
watchdog() {
    ...
    while true; do
        ...
        if is_process_alive; then
            ...
            LAST_FOREGROUND_ACTIVITY="Roblox process not running"
            LAST_FOREGROUND_IS_ROBLOX=0
            LAST_DETECTION_REASON="Roblox process is not running"
        fi
        ...
        if [ "$NOT_IN_GAME_STREAK" -ge "$OUT_OF_GAME_GRACE" ]; then
            ...
            if [ "$RECOVERY_RUNNING" -ne 0 ]; then
                ...
                echo "[rejoin] blocked: recovery already running."

            elif [ "$SINCE_JOIN" -lt "$REJOIN_COOLDOWN" ]; then

                echo "[rejoin] blocked: cooldown ${SINCE_JOIN}s / ${REJOIN_COOLDOWN}s since last join."

            else

                echo "=========================================="
                echo "[rejoin] OUT OF GAME DETECTED"
                echo "=========================================="
                echo "[rejoin] Out of game streak: ${NOT_IN_GAME_STREAK}s"
                echo "[rejoin] Foreground: ${LAST_FOREGROUND_ACTIVITY}"
                echo "[rejoin] Roblox foreground: ${LAST_FOREGROUND_IS_ROBLOX}"
                echo "[rejoin] CPU: ${CPU_NOW}%"
                echo "[rejoin] UDP: ${UDP_NOW}"
                echo "[rejoin] Reason: ${LAST_DETECTION_REASON}"
                echo "[rejoin] Sending join command..."
                echo "=========================================="
        ...
        if [ $((NOW - LAST_STATUS_TS)) -ge "$STATUS_EVERY" ]; then
            ...
            echo "[status] state=${GAME_STATE} cpu=${CPU_NOW}% udp=${UDP_NOW} foreground=${LAST_FOREGROUND_ACTIVITY} roblox_foreground=${LAST_FOREGROUND_IS_ROBLOX} streak=${NOT_IN_GAME_STREAK}/${OUT_OF_GAME_GRACE} since_join=$((NOW - LAST_JOIN_TIME))s reason=${LAST_DETECTION_REASON} iter=${ITER_MS}s"
